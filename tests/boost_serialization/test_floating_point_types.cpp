...
TEST(boost_serialization, writes_float_and_double) {
  const FloatAndDoubleStruct s{1.0f, 2.0};

  const auto bytes = rfl::boost_serialization::write(s);
  const auto res = rfl::boost_serialization::read<FloatAndDoubleStruct>(bytes);
