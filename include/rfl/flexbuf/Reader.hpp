...
struct Reader {
  ...
  rfl::Result<T> to_basic_type(const InputVarType& _var) const noexcept {
    if constexpr (std::is_same<std::remove_cvref_t<T>, std::string>()) {
      if (!_var.IsString()) {
        return rfl::Unexpected(rfl::Error("Could not cast to a string."));
      }
      return std::string(_var.AsString().c_str());
    } else if constexpr (std::is_same<std::remove_cvref_t<T>,
                                      rfl::Bytestring>()) {
      if (!_var.IsBlob()) {
        return rfl::Unexpected(rfl::Error("Could not cast to a bytestring."));
      }
      ...
      if (!_var.IsNumeric()) {
        return rfl::Unexpected(rfl::Error("Could not cast to double."));
      }
      return static_cast<T>(_var.AsDouble());
    } else if constexpr (std::is_integral<std::remove_cvref_t<T>>()) {
      if (!_var.IsNumeric()) {
        return rfl::Unexpected(rfl::Error("Could not cast to int."));
      }
