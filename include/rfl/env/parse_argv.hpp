#ifndef RFL_ENV_PARSE_ARGV_HPP_
#define RFL_ENV_PARSE_ARGV_HPP_
...
/// Returns true if the token looks like a ENV option (starts with '-'
...
/// - Positional arguments: bare arguments not prefixed with dashes (stored in
/// `positional`) The special argument "--" forces all subsequent arguments to
/// be treated as positional.
/// @param argc Number of command-line arguments
/// @param argv Array of command-line argument strings
/// @return A Result containing ParsedArgs with categorized arguments or an
/// error
...
inline rfl::Result<ParsedArgs> parse_argv(int argc, char* argv[]) {
  ...
  for (size_t i = 0; i < args.size(); ++i) {
    ...
    if (arg_raw.starts_with("--")) {
      ...
      auto key =
          std::string(eq == std::string_view::npos ? arg : arg.substr(0, eq));
      auto val =
          std::string(eq == std::string_view::npos ? "" : arg.substr(eq + 1));
      if (key.empty()) {
    ...
    if (arg_raw.size() >= 2 && arg_raw[0] == '-') {
      ...
      if (eq != std::string_view::npos) {
        const auto r = add_short(std::string(arg.substr(0, eq)),
                                 std::string(arg.substr(eq + 1)), arg_raw);
        if (!r) {
          return error(r.error().what());
        }
        continue;
      }
