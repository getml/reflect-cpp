#ifndef RFL_ENV_RESOLVE_ARGS_HPP_
#define RFL_ENV_RESOLVE_ARGS_HPP_
...
void collect_positional_names(std::vector<std::string>& _names,
  ...
  if constexpr (sizeof...(_is) > 0) {
    collect_positional_names<NamedTupleType>(_names,
                                             std::index_sequence<_is...>{});
  }
}

template <class NamedTupleType>
void collect_positional_names(std::vector<std::string>&,
                              std::index_sequence<>) {}
...
rfl::Result<bool> collect_short_mapping(
  ...
  if constexpr (Info::is_short) {
    ...
    if (!_mapping.emplace(short_str, long_str).second) {
      return rfl::error("Duplicate short name '-" + short_str +
                        "' in struct definition.");
    }
  }
  if constexpr (sizeof...(_is) > 0) {
    return collect_short_mapping<NamedTupleType>(_mapping,
                                                 std::index_sequence<_is...>{});
  }
  return true;
}
...
template <class NamedTupleType>
rfl::Result<bool> collect_short_mapping(std::map<std::string, std::string>&,
                                        std::index_sequence<>) {
  return true;
}
...
template <class NamedTupleType, size_t _i, size_t... _is>
void collect_short_bool_names(std::set<std::string>& _names,
                              std::index_sequence<_i, _is...>) {
  ...
  if constexpr (sizeof...(_is) > 0) {
    collect_short_bool_names<NamedTupleType>(_names,
                                             std::index_sequence<_is...>{});
  }
}

template <class NamedTupleType>
void collect_short_bool_names(std::set<std::string>&, std::index_sequence<>) {}
...
/// This function:
/// 1. Maps positional arguments to their corresponding field names based on
/// declaration order
/// 2. Expands short arguments (e.g., -p) to long names (e.g., --port) using
/// Short<> annotations
...
rfl::Result<std::map<std::string, std::string>> resolve_args(
  ...
  if (!reclaimed.empty()) {
    _parsed.positional.insert(_parsed.positional.begin(),
  ...
  for (size_t i = 0; i < _parsed.positional.size(); ++i) {
    ...
    if (result.count(long_name)) {
      return rfl::error("Conflict: positional argument and '--" + long_name +
                        "' both provided.");
    }
  ...
  for (auto& [short_name, value] : _parsed.short_args) {
    ...
    if (result.count(long_name)) {
      return rfl::error("Conflict: '-" + short_name + "' and '--" + long_name +
                        "' both provided.");
    }
