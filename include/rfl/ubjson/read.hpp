...
/// Parses an object from UBJSON bytes using reflection (contiguous container
/// version). UBJSON (Universal Binary JSON) is a compact binary format that
/// mirrors JSON's data model. It's designed to be faster to parse than JSON
/// while maintaining compatibility with JSON semantics.
/// @tparam T The type to parse into
/// @tparam Ps Processors to apply during parsing (transforms the data)
/// @param _bytes A contiguous byte container (e.g., std::vector<uint8_t>,
/// std::string) containing UBJSON data
/// @return Result containing either the parsed object (or array of objects) or
/// an error message
