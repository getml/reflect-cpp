#ifndef RFL_PARQUET_READER_HPP_
#define RFL_PARQUET_READER_HPP_
...
struct Reader {
  using InputArrayType = parquet_object_array;
  using InputObjectType = parquet_object_map;
  using InputVarType = parquet_object;

  template <class T>
  static constexpr bool has_custom_constructor =
      (requires(InputVarType var) { T::from_parquet_obj(var); });
  ...
  rfl::Result<InputVarType> get_field_from_object(
    ...
    for (uint32_t i = 0; i < _obj.size; ++i) {
      ...
      if (key.type != PARQUET_OBJECT_STR) {
  ...
  bool is_empty(const InputVarType& _var) const noexcept {
    return _var.type == PARQUET_OBJECT_NIL;
  }
  ...
  rfl::Result<T> to_basic_type(const InputVarType& _var) const noexcept {
    ...
    if constexpr (std::is_same<std::remove_cvref_t<T>, std::string>()) {
      ...
      if (type != PARQUET_OBJECT_BOOLEAN) {
        return error("Could not cast to boolean.");
      }
      return _var.via.boolean;

    } else if constexpr (std::is_floating_point<std::remove_cvref_t<T>>()) {
      if (type == PARQUET_OBJECT_FLOAT32 || type == PARQUET_OBJECT_FLOAT64 ||
          type == PARQUET_OBJECT_FLOAT) {
      ...
      if (type == PARQUET_OBJECT_POSITIVE_INTEGER) {
        return static_cast<T>(_var.via.u64);
      } else if (type == PARQUET_OBJECT_NEGATIVE_INTEGER) {
  ...
  rfl::Result<T> use_custom_constructor(
    ...
    try {
      return T::from_parquet_obj(_var);
