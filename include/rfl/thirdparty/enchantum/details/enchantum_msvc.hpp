...
constexpr auto type_name_func_size() noexcept {
  ...
  return SZC(__FUNCSIG__) -
         SZC("auto __cdecl enchantum::details::type_name_func_size<enum ") -
         SZC(">(void) noexcept");
}
...
constexpr auto enum_in_array_name() noexcept {
  string_view s =
      __FUNCSIG__ +
      sizeof("auto __cdecl enchantum::details::enum_in_array_name<") - 1;
...
constexpr auto reflect() noexcept {
  ...
  constexpr auto elements =
      details::get_elements<E, Pair, details::generate_arrays<E, Min, Max>(),
                            ShouldNullTerminate>();
