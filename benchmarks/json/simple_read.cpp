...
static void read_using_yyjson_comparison() {
  ...
  const auto view_reader =
      rfl::parsing::ViewReader<rfl::json::Reader, rfl::json::Writer,
                               decltype(view), rfl::Processors<>>(
          &reader, &view, &found, &set, &errors);

  reader.read_object(view_reader, rfl::json::Reader::InputObjectType(root));
  if (!std::get<0>(found)) {
...
static void read_using_yyjson_comparison2() {
  ...
  auto view = View{&homer.first_name, &homer.last_name, &homer.age};
  ...
  // std::get<0>(view)->~string();
  // std::get<1>(view)->~string();
