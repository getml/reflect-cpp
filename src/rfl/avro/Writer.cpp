...
Writer::OutputArrayType Writer::add_array_to_map(
  ...
  avro_value_t new_array;
  int result = avro_value_add(&_parent->val_, _name.data(), &new_array, nullptr, nullptr);
  if (result != 0) {
    throw std::runtime_error(std::string(__FUNCTION__) + " error("+ std::to_string(result)+"): "  + avro_strerror());
  }
  return OutputArrayType{new_array};
...
Writer::OutputArrayType Writer::add_array_to_object(
  ...
  avro_value_t new_array;
  int result = avro_value_get_by_name(&_parent->val_, _name.data(), &new_array, nullptr);
  if (result != 0) {
    throw std::runtime_error(std::string(__FUNCTION__) + " error("+ std::to_string(result)+"): "  + avro_strerror());
  }
  return OutputArrayType{new_array};
...
Writer::OutputMapType Writer::add_map_to_map(
    ...
    OutputMapType* _parent) const {
  avro_value_t new_map;
  int result = avro_value_add(&_parent->val_, _name.data(), &new_map, nullptr, nullptr);
  if (result != 0) {
    throw std::runtime_error(std::string(__FUNCTION__) + " error("+ std::to_string(result)+"): "  + avro_strerror());
  }
  return OutputMapType{new_map};
...
Writer::OutputMapType Writer::add_map_to_object(
    ...
    OutputObjectType* _parent) const {
  avro_value_t new_map;
  int result = avro_value_get_by_name(&_parent->val_, _name.data(), &new_map, nullptr);
  if (result != 0) {
    throw std::runtime_error(std::string(__FUNCTION__) + " error("+ std::to_string(result)+"): "  + avro_strerror());
  }
  return OutputMapType{new_map};
...
Writer::OutputObjectType Writer::add_object_to_map(
    ...
    OutputMapType* _parent) const {
  avro_value_t new_object;
  int result = avro_value_add(&_parent->val_, _name.data(), &new_object, nullptr, nullptr);
  if (result != 0) {
    throw std::runtime_error(std::string(__FUNCTION__) + " error("+ std::to_string(result)+"): "  + avro_strerror());
  }
  return OutputObjectType{new_object};
...
Writer::OutputObjectType Writer::add_object_to_object(
    ...
    OutputObjectType* _parent) const {
  avro_value_t new_object;
  int result = avro_value_get_by_name(&_parent->val_, _name.data(), &new_object, nullptr);
  if (result != 0) {
    throw std::runtime_error(std::string(__FUNCTION__) + " error("+ std::to_string(result)+"): "  + avro_strerror());
  }
  return OutputObjectType{new_object};
...
Writer::OutputUnionType Writer::add_union_to_map(
  ...
  avro_value_t new_union;
  int result = avro_value_add(&_parent->val_, _name.data(), &new_union, nullptr, nullptr);
  if (result != 0) {
    throw std::runtime_error(std::string(__FUNCTION__) + " error("+ std::to_string(result)+"): "  + avro_strerror());
  }
  return OutputUnionType{new_union};
...
Writer::OutputUnionType Writer::add_union_to_object(
  ...
  avro_value_t new_union;
  int result = avro_value_get_by_name(&_parent->val_, _name.data(), &new_union, nullptr);
  if (result != 0) {
    throw std::runtime_error(std::string(__FUNCTION__) + " error("+ std::to_string(result)+"): "  + avro_strerror());
  }
  return OutputUnionType{new_union};
...
Writer::OutputVarType Writer::add_null_to_map(
    const std::string_view& _name, OutputMapType* _parent) const {
  avro_value_t new_null;
  int result = avro_value_add(&_parent->val_, _name.data(), &new_null, nullptr, nullptr);
  if (result != 0) {
    throw std::runtime_error(std::string(__FUNCTION__) + " error("+ std::to_string(result)+"): "  + avro_strerror());
  }
  result = avro_value_set_null(&new_null);
...
Writer::OutputVarType Writer::add_null_to_object(
    const std::string_view& _name, OutputObjectType* _parent) const {
  avro_value_t new_null;
  int result = avro_value_get_by_name(&_parent->val_, _name.data(), &new_null, nullptr);
  if (result != 0) {
    throw std::runtime_error(std::string(__FUNCTION__) + " error("+ std::to_string(result)+"): "  + avro_strerror());
  }
  result = avro_value_set_null(&new_null);
